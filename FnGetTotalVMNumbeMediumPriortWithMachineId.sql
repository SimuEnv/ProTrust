...
/****** Object:  UserDefinedFunction [dbo].[FnGetTotalVMNumberPriortWithMachineId]    Script Date: 1/20/2018 5:23:59 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE FUNCTION [dbo].[FnGetTotalVMNumbeMediumPriortWithMachineId](@mach_id bigint, @tm bigint, @event_type int) 
RETURNS float
AS
BEGIN 
		  ...
		  ELSE  
			(select COUNT(*)  from 
				(select distinct te.job_id, te.task_index from [dbo].Failed_hosts_task_usage as tu
					...
					and te.event_type = @event_type and te.priority in (2,3,4,5,6,7,8)
